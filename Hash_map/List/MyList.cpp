...
list_code list_construct(List *that_list, const size_t list_size)
{
    ...
    that_list->data[0] = neutral;
    that_list->next[0] = 0;
    that_list->prev[0] = 0;
    for (size_t i = 1; i <= list_size; i++)
    {
        that_list->data[i] = neutral;
...
list_code list_destruct(List *that_list)
{
    ...
    if (that_list->data)
    {
        for (size_t i = 1; i < that_list->length; i++)
            free(that_list->data[i]);
        free(that_list->data);
    }
    if (that_list->next)
        free(that_list->next);
...
list_code list_remove(List *that_list, const size_t physical_index)
{
    ...
    free(that_list->data[physical_index]);
...
{
    return list_remove(that_list, that_list->next[0]);
}

list_code list_remove_back(List *that_list)
{
    return list_remove(that_list, that_list->prev[0]);
}
...
list_code list_get_next_index(List *that_list, size_t physical_index, list_elem_type *next_index)
{
    ...
    *next_index = that_list->data[that_list->next[physical_index]];
...
list_code list_get_prev_index(List *that_list, size_t physical_index, list_elem_type *prev_index)
{
    ...
    *prev_index = that_list->data[that_list->prev[physical_index]];
...
void list_print_list_appearance(List *that_list, list_code code, const list_functions function)
{
    ...
    fprintf(log, "%s", list_state_text[code]);
    if (that_list->sorted)
    ...
    fprintf(log, "CURRENT CAPACITY IS %lu\n", that_list->capacity);
    fprintf(log, "CURRENT SIZE IS            %lu\n", that_list->length);
    ...
    sprintf(picture_adding, "%ld.txt", doing);
